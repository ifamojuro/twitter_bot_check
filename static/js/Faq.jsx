...
class Faq extends React.Component {
	constructor(props){
		super(props)
		this.questions = [
		{"Question":"What is Analyze.me?", "Answer":"Analyze.me is a web applet which allows users to scan the accounts that they follow on Twitter in order to dectect if any of those accounts are content polluters!"},
		{"Question":'What is a "Content Polluting"?', "Answer":'"Content Polluters" (often reffered to as "scammers", or "bots") any entity which uses social media to spread disinformation. Content Polluters may be a physical person, or an automated program.'},
		{"Question":"Wow! How does Analyze.me work?", "Answer":"Analyze.me uses data from labeled Content Polluters to their isolate indentifying features. The applet then uses these features to create Logisitic Classifier which can predict if an account is a Content Polluter. When a user uses Analyze.me, the applet sends a call to the Twitter API in order to find information about the accounts that that user follows. After the user authenticates Analyze.me, the applet compares the followed accounts with the classifier in order to make predictions."},
		...
		{"Question":"Why does Analyze request so many permissions from Twitter account?", "Answer":"Analyze.me currently works with the Twitter API to analyze user data. The Twitter API does not allow apps to select specifc permission, but instead buckets permissions into 'read' and 'write' buckets. The permissions that analyze.me requests fall under the 'read' bucket. "},
		...
		{"Question":"How can I build my own version of Analyze.me/something like Analyze.me", "Answer":"There are tons of resources online that can help anyone build a logistic classifier. As a python developer, I recomend this one!"},
		]
	}
